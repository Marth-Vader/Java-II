...
public class InfantFrame extends JFrame {
    ...
    private class FileMenuBar extends JMenuBar {
        ...
        public FileMenuBar() {
            ...
            // DONE: create and add menu items
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (returnVal == JFileChooser.APPROVE_OPTION) {
                        ...
                        try {
                            ...
                            // DONE: Do the loading work: the first two
                            ...
                            // DONE: If we did not load any data then open a
                            // message dialog box to indicate an error
                            // if infant has no trials, we loaded no data
                            if (infant.getSize() == 0) {
                                // Catch IO errors
                                JOptionPane.showMessageDialog(fileChooser, "No data loaded");
                                // Return the cursor to standard form
                                InfantFrame.this.setCursor(null);
                            }

                            // DONE: Update the frame
    ...
    public class DataPanel extends JPanel {
        ...
        private DataPanel() {
            ...
            this.runButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    // TO DO: implement
                    // if an infant and trial is selected
                    if (infantTextField != null) {

                        // if button is pressed and the text is Start
                        if (runButton.getText().equals("Start")) {

                            // if the runButton button is pressed, then start
                            // the timer
                            timer.start();

                            // if runButton is pressed, change the text to Stop
                            runButton.setText("Stop");

                        }

                        // if button is pressed and text is Stop
                        else {
                            // stop the timer
                            timer.stop();

                            // change the text on the button to Start
        ...
        public void setTime(int newTime) {
            ...
            if (InfantFrame.this.trial != null) {
                ...
                // if the new time is within the allowable range
                if ((newTime >= 0) && (newTime <= 15000)) {
                    // reassign the currentTime
                    currentTime = newTime;
                    // and then set the timeSlider to the new currentTime
                    timeSlider.setValue(currentTime);

                    // extract state for current time
                    // force an update of the display
                    update(trial.getItem(timeSlider.getValue()));
                ...
                // if time is out of range
                    ...
                    //set the runButton text to Start
                    runButton.setText("Start");
                    // turn the timer off
                    timer.stop();
            ...
            // deal w case that trial is null
            else {
        ...
        public KinematicPointAbstract createKinematicModel() {
            ...
            //left hip
            KinematicPointConstant leftHip = new KinematicPointConstant(red, LINE_WIDTH, 0.0, 0.05, 0.0);
            //right hip
            KinematicPointConstant rightHip = new KinematicPointConstant(red, LINE_WIDTH, 0.0, -0.05, 0.0);
            ...
            KinematicPointState upperBack = new KinematicPointState(blue, LINE_WIDTH, "upper_back");
            lowerBack.addChild(upperBack);
            
            //left leg
            //create the left knee
            KinematicPointState leftKnee = new KinematicPointState(blue, LINE_WIDTH, "left_knee");
            //add the left knee to the left hip
            leftHip.addChild(leftKnee);
            //create the left ankle
            KinematicPointState leftAnkle = new KinematicPointState(red, LINE_WIDTH, "left_ankle");
            //add the left ankle to the left knee
            leftKnee.addChild(leftAnkle);
            //create the left foot
            KinematicPointState leftFoot = new KinematicPointState(blue, LINE_WIDTH, "left_foot");
            //add the left foot to the left ankle
            leftAnkle.addChild(leftFoot);
            
            
            //right leg
            //create the right knee
            KinematicPointState rightKnee = new KinematicPointState(blue, LINE_WIDTH, "right_knee");
            //add the left knee to the left hip
            rightHip.addChild(rightKnee);
            //create the left ankle
            KinematicPointState rightAnkle = new KinematicPointState(red, LINE_WIDTH, "right_ankle");
            //add the left ankle to the left knee
            rightKnee.addChild(rightAnkle);
            //create the left foot
            KinematicPointState rightFoot = new KinematicPointState(blue, LINE_WIDTH, "right_foot");
            //add the left foot to the left ankle
            ...
            //left shoulder
            //create the left knee
            KinematicPointState leftShoulder = new KinematicPointState(red, LINE_WIDTH, "left_shoulder");
            //add the left knee to the left hip
            upperBack.addChild(leftShoulder);
            //create the left ankle
            KinematicPointState leftElbow = new KinematicPointState(blue, LINE_WIDTH, "left_elbow");
            //add the left ankle to the left knee
            leftShoulder.addChild(leftElbow);
            //create the left foot
            KinematicPointState leftWrist = new KinematicPointState(red, LINE_WIDTH, "left_wrist");
            //add the left foot to the left ankle
            ...
            //right shoulder
            //create the left knee
            KinematicPointState rightShoulder = new KinematicPointState(red, LINE_WIDTH, "right_shoulder");
            //add the left knee to the left hip
            upperBack.addChild(rightShoulder);
            //create the left ankle
            KinematicPointState rightElbow = new KinematicPointState(blue, LINE_WIDTH, "right_elbow");
            //add the left ankle to the left knee
            rightShoulder.addChild(rightElbow);
            //create the left foot
            KinematicPointState rightWrist = new KinematicPointState(red, LINE_WIDTH, "right_wrist");
            //add the left foot to the left ankle
            rightElbow.addChild(rightWrist);
            

            //return the completed baby
            return root;
        }
        ...
        private void update(State state) {
            // TO DO: implement
            // set the time text field

            //set the time field
            timeTextField.setText("Time Step: " + Integer.toString(currentTime));
            // set the infantID text field
            infantTextField.setText("Infant: " + infant.getInfantID());

            //update each of the kinematic panels
            topViewPanel.setState(state);
            sideViewPanel.setState(state);
            rearViewPanel.setState(state);
            
            //force all the components to redraw
            this.repaint();
